...
describe("DCAUserController", () => {
  ...
  describe("getMyPlans", () => {
    it("should return user plans", async () => {
      ...
      (DCAPlanRepository.prototype.findByUser as jest.Mock).mockResolvedValue(mockPlans);

      req.params = { userAddress: "0x123" };

      await DCAUserController.getMyPlans(req as Request, res as Response);
  ...
  describe("getMyPlanDetails", () => {
    it("should return plan details if user owns the plan", async () => {
      ...
      await DCAUserController.getMyPlanDetails(req as Request, res as Response);

...
export class DCAPlanRepository {
    ...
    async markAsCompleted(planId) {
        return await DCAPlanModel.findByIdAndUpdate(planId, {
            status: "completed",
            isActive: false,
            updatedAt: new Date(),
        }, { new: true });
    }
    async updateNextExecution(planId, data) {
        ...
        if (data.executedOperations !== undefined) {
            ...
            const nextExecution = newExecutedOps < plan.totalOperations
        ...
        plan.updatedAt = now;
        logger.info(`[DCAPlanRepository] Updated execution for ${planId}: ${plan.executedOperations}/${plan.totalOperations}`, { service: 'System', method: 'Repository' });

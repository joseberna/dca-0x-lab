...
export class DCAService {
    constructor() {
        ...
        if (!rpcUrl)
            throw new Error(`RPC URL not configured for network: ${activeNetwork}`);
        if (!accountingAddress)
            throw new Error(`Accounting address not configured for network: ${activeNetwork}`);
        this.provider = new ethers.providers.JsonRpcProvider(rpcUrl);
        const privateKey = process.env.PRIVATE_KEY;
        if (!privateKey)
            throw new Error("PRIVATE KEY no configurada");
        this.wallet = new ethers.Wallet(privateKey, this.provider);
        this.accountingContract = new ethers.Contract(accountingAddress, DCA_ACCOUNTING_ABI, this.wallet);
    ...
    async createPlanOnChain(params) {
        ...
        const approveTx = await usdcContract.approve(this.accountingContract.address, totalAmount.toString());
        ...
        const tx = await this.accountingContract.createPlan(toToken, // NEW: Pass token symbol
        totalAmount.toString(), amountPerInterval.toString(), intervalSeconds, totalOperations);
        ...
        const accountingLogs = receipt.logs.filter((log) => log.address.toLowerCase() === this.accountingContract.address.toLowerCase());
    ...
    async syncPlanFromChain(txHash) {
        try {
    ...
    // ===========================================================
    async executeTickOnChain(mongoPlanId) {

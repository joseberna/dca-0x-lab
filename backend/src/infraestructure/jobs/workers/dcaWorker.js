...
export const dcaWorker = new Worker("dca-executions", async (job) => {
    ...
    try {
        ...
        if (plan.executedOperations < plan.totalOperations) {
            const nextExecution = new Date(Date.now() + plan.intervalSeconds * 1000);
            await planRepository.updatePlan(planId, {
                isActive: true,
                nextExecution,
            });
            logger.info(`[INFO] Plan ${planId} programado nuevamente para ${nextExecution.toISOString()}`, { service: 'System', method: 'Worker' });
        }
        else {

...
export async function sendTransaction(txData: any): Promise<string> {
    try {
        ...
        if (!tx.to || !tx.data) throw new Error("⚠️ [ST] Incomplete tx object");

        if (!txData) throw new Error("⚠️ [ST] Incomplete tx object");

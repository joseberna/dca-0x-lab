...
const routerAbiPath = path.resolve(__dirname, "../../infraestructure/blockchain/abi/SwapRouter.json");
const SwapRouterABI = JSON.parse(fs.readFileSync(routerAbiPath, "utf-8"));
const erc20ABI = [
    "function allowance(address owner, address spender) view returns (uint256)",
    "function approve(address spender, uint256 value) returns (bool)"
];
export async function gaslessDCA(plan) {
    ...
    const tokenOut = plan.tokenTo === "WETH"
        ? ethers.utils.getAddress(process.env.SM_WETH_POLYGON)
        : ethers.utils.getAddress(process.env.SM_WBTC_POLYGON);
    const amountIn = ethers.utils.parseUnits(plan.amountPerInterval.toString(), 6);
    // 1. Encode approval

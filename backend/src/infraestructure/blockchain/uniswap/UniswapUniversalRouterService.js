...
export class UniswapUniversalRouterService {
    constructor() {
        ...
        this.universalRouter = new ethers.Contract(process.env.UNISWAP_UNIVERSAL_ROUTER, UniversalABI, // ⬅️ SOLO EL ARRAY ABI
        this.wallet);
        logger.info("🧩 Universal Router inicializado");
    }
    async swapExactInputUSDCtoWBTC(amountUSDC) {
        ...
        const inputs = [
            ethers.utils.defaultAbiCoder.encode(["address", "address", "uint256"], [
                process.env.SM_USDC_POLYGON,
                process.env.SM_WBTC_POLYGON,
                amountIn
            ])
        ];
        const tx = await this.universalRouter.execute(commands, inputs, { value: 0 });

...
export class UniswapPolygonSmartRouterService {
    ...
    async executeSwapUSDCtoWBTC(amountUSDC) {
        ...
        // USDC → raw amount
        const amountIn = CurrencyAmount.fromRawAmount(this.USDC, JSBI.BigInt(Math.floor(amountUSDC * 1e6)) // USDC tiene 6 decimales
        );
        const slippage = new Percent(JSBI.BigInt(100), JSBI.BigInt(10000)); // 1%
        // MULTIHOP ROUTE
        const route = await this.router.route(amountIn, this.WBTC, SwapType.UNIVERSAL_ROUTER, {
            recipient: this.wallet.address,
            slippageTolerance: slippage,
            deadline: Math.floor(Date.now() / 1000) + 1800,
        });
        if (!route)
            throw new Error("❌ Uniswap no encontró ruta multihop");
